...
const Token = require('./build/contracts/GreenTeaToken.json');
...
const mainAccount = {
    ...
    TEAaddress: "0x2849b1ae7e04a3d9bc288673a92477cf63f28af4"
}
...
const testAccount = {
    ...
    TEAaddress: "0xa0286abcdf5ac136cdbea4fecea0f83c093e5107"
}
...
async function deployMasterChef(tokenContractAddress, account, nonce) {
    ...
    const teaPerBlock = web3.utils.toWei('1', 'ether');

    MasterChefContract.deploy({
        data: MasterChef.bytecode,
        arguments: [tokenContractAddress, account.address, account.address, teaPerBlock, 6347879]
    })
        .send({
            nonce: web3.utils.toHex(nonce++),
...
async function setupSmartChef() {
    ...
    const TEAaddress = "0x2849b1aE7E04A3D9Bc288673A92477CF63F28aF4" // TEA
    ...
    deploySmartChef(TEAaddress, rewardToken, rewardAmount.toString(), rewardPerBlock.toString(), account, nonce, startBlock, burnMultiplier, endBlock);

    // deploy a new TEA Token
    /* TokenContract.deploy({
        data: Token.bytecode,
    })
        .send({
        ...
        .then((teaContractAddress) => {
            console.log(`Token contract deployed at ${teaContractAddress.options.address}`);
            deploySmartChef(teaContractAddress.options.address, rewardToken, rewardAmount, rewardPerBlock, account, nonce);
        }); */
}
...
async function startSmartFarming() {
    ...
    web3.eth.sendSignedTransaction(signed.rawTransaction)
        ...
        .on('receipt', function (receipt) {
            var transactionHash = receipt.transactionHash;
            console.log('receipt', receipt);
            depositTeaToSmartChef(smartChef)
        });
}

async function depositTeaToSmartChef(smartChef) {
    const accountData = mainAccount
    const web3 = accountData.web3
    const privateKey = accountData.privateKey
    const account = web3.eth.accounts.privateKeyToAccount('0x' + privateKey);
    ...
    // deposit TEA to smartchef
    const TEAtoSend = web3.utils.toWei('0.1', 'ether');
    const txOptions2 = {
        nonce: web3.utils.toHex(nonce),
        from: mainAccount.address,
        to: smartChef,
        gas: web3.utils.toHex(config.gasLimit),
        gasPrice: web3.utils.toHex(config.gasPrice),
        data: SmartChefContract.methods.deposit(TEAtoSend).encodeABI()
    }
    const signed2 = await web3.eth.accounts.signTransaction(txOptions2, account.privateKey);
    web3.eth.sendSignedTransaction(signed2.rawTransaction)
        .on('error', function (err) {
 ...
 async function setupModSalary() {
    ...
    const TEAaddress = "0x2849b1aE7E04A3D9Bc288673A92477CF63F28aF4" // main net

    deployModSalary(TEAaddress, account, nonce);
}
